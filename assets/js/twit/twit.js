...
getTwitData(`/api/twits/getReceive?page=${twit_page}`, '보낸사람', '받은시간');

// 받은쪽지 탭 클릭
$('#receive').click(function () {
  window.history.pushState({}, '', `/views/twit/twit.html?page=1`);
  getTwitData(`/api/twits/getReceive?page=1`, '보낸사람', '받은시간');
})

// 보낸 쪽지 탭 클릭
$('#send').click(function () {
  window.history.pushState({}, '', `/views/twit/twit.html?page=1`);
  getTwitData(`/api/twits/getSend?page=1`, '받은사람', '보낸시간');
})
  ...
  try {
    ...
    mainDom.innerHTML = list
      .map(twit => {
        ...
        const name = url === `/api/twits/getSend?page=${twit_page}` ? senderName : receiverName;
